...
public class Phno extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		this.setTitle("Authenticating Phone Numbers");
		setContentView(R.layout.activity_phno);
		
		final SQLiteDatabase sampleDB =  this.openOrCreateDatabase(SAMPLE_DB_NAME, MODE_PRIVATE, null);
		String query = "Select distinct * FROM " + SAMPLE_TABLE_NAME2;
		...
		Button b2=(Button)findViewById(R.id.button2);
		 b1.setOnClickListener(new OnClickListener() {
	    		...
	    		public void onClick(View v) {
	    			...
	    			if(cb1.isChecked()){
	    				sampleDB.execSQL("DELETE FROM " +SAMPLE_TABLE_NAME2 +" WHERE Nums='"+cb1.getText()+"'");
	    			}
	    			if(cb2.isChecked()){
	    				sampleDB.execSQL("DELETE FROM " +SAMPLE_TABLE_NAME2 +" WHERE Nums='"+cb2.getText()+"'");
	    			} 
	    			if(cb3.isChecked()){
	    				sampleDB.execSQL("DELETE FROM " +SAMPLE_TABLE_NAME2 +" WHERE Nums='"+cb3.getText()+"'");
	    			} 
	    			if(cb4.isChecked()){
	    				sampleDB.execSQL("DELETE FROM " +SAMPLE_TABLE_NAME2 +" WHERE Nums='"+cb4.getText()+"'");
	    			} 
	    			if(cb5.isChecked()){
	    				sampleDB.execSQL("DELETE FROM " +SAMPLE_TABLE_NAME2 +" WHERE Nums='"+cb5.getText()+"'");
	    			} 
	    			if(cb6.isChecked()){
	    				sampleDB.execSQL("DELETE FROM " +SAMPLE_TABLE_NAME2 +" WHERE Nums='"+cb6.getText()+"'");
	    			} 
	    			if(cb7.isChecked()){
	    				sampleDB.execSQL("DELETE FROM " +SAMPLE_TABLE_NAME2 +" WHERE Nums='"+cb7.getText()+"'");
	    			} 
	    			if(cb8.isChecked()){
	    				sampleDB.execSQL("DELETE FROM " +SAMPLE_TABLE_NAME2 +" WHERE Nums='"+cb8.getText()+"'");
	    			} 
	    			if(cb1.isChecked()){
	    				sampleDB.execSQL("DELETE FROM " +SAMPLE_TABLE_NAME2 +" WHERE Nums='"+cb1.getText()+"'");
	    			} 
	    		...
	    		Intent i= new Intent(Phno.this, PhnoDummy.class);	
	    		startActivity(i);
	    		}
		 ...
		 b2.setOnClickListener(new OnClickListener() {
	    		...
	    		public void onClick(View v) {
	    			...
	    			Intent i= new Intent(Phno.this, MainActivity.class);	
		    		startActivity(i);
		    		}
	...
	public boolean onCreateOptionsMenu(Menu menu) {
		...
		getMenuInflater().inflate(R.menu.activity_phno, menu);
		return true;
	}
